...
function LayoutContent({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen font-sans bg-slate-50 dark:bg-black overflow-hidden relative">
      
      {/* Luminous Background Orbs */}
      <div className="absolute top-[-20%] left-[-10%] w-[50%] h-[50%] rounded-full bg-blue-400/30 dark:bg-indigo-900/40 blur-[120px] pointer-events-none" />
      <div className="absolute bottom-[-20%] right-[-10%] w-[60%] h-[60%] rounded-full bg-purple-400/20 dark:bg-purple-900/30 blur-[120px] pointer-events-none" />

      {/* Context-Aware Sidebar */}
      <aside className={`${isSidebarCollapsed ? 'w-16' : 'w-64'} bg-white/60 dark:bg-zinc-950/40 backdrop-blur-2xl border-r border-white/40 dark:border-white/10 flex flex-col hidden md:flex shadow-xl shadow-blue-900/5 transition-all duration-300 z-10`}>
              ...
              <Link
                ...
                className={`group flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 relative overflow-hidden ${
                  isActive 
                    ...
                    : "text-zinc-600 dark:text-zinc-400 hover:bg-white/50 dark:hover:bg-zinc-800/50 hover:text-zinc-900 dark:hover:text-zinc-200 font-medium"
                }`}
              >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* Sticky Top Header */}
        <header className="h-[88px] bg-white/60 dark:bg-zinc-950/40 backdrop-blur-2xl border-b border-white/40 dark:border-white/10 flex items-center justify-between px-6 shadow-sm z-20 shrink-0">
            <div className="flex items-center gap-4">
              <button 
                onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)} 
                className="p-2 -ml-2 hover:bg-white/80 dark:hover:bg-zinc-800/50 rounded-xl text-zinc-600 dark:text-zinc-300 transition-all duration-200 hidden md:flex items-center justify-center hover:shadow-md"
            ...
            <div className="flex items-center gap-6">
              {/* Home Icon */}
              <Link href="/" className="flex items-center gap-2 text-zinc-600 hover:text-blue-600 dark:text-zinc-300 dark:hover:text-blue-400 transition-all duration-300 bg-white/50 dark:bg-zinc-800/30 hover:bg-white dark:hover:bg-zinc-800 backdrop-blur-md px-3 py-2 rounded-xl font-medium text-sm shadow-sm hover:shadow-md border border-white/40 dark:border-white/5">
                <span>🏠</span> Home
              </Link>
              ...
              <div className="relative group">
                <button className="flex items-center gap-2 text-zinc-600 hover:text-indigo-600 dark:text-zinc-300 dark:hover:text-indigo-400 transition-all duration-300 bg-white/50 dark:bg-zinc-800/30 hover:bg-white dark:hover:bg-zinc-800 backdrop-blur-md px-3 py-2 rounded-xl font-bold text-sm shadow-sm hover:shadow-md border border-white/40 dark:border-white/5">
                  📅 {getActiveYear()?.name}
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m6 9 6 6 6-6"/></svg>
                </button>
                {/* Dropdown */}
                <div className="absolute right-0 mt-3 w-48 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-2xl rounded-2xl shadow-2xl border border-white/40 dark:border-zinc-700/50 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50 transform origin-top-right scale-95 group-hover:scale-100 overflow-hidden">
                  <div className="p-3 border-b border-zinc-100 dark:border-zinc-800">
                    <p className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Select Academic Year</p>
                  </div>
                  <div className="max-h-60 overflow-y-auto p-1">
                    {academicYears.map(year => (
                      <button 
                        ...
                        className={`w-full text-left px-3 py-2 rounded-xl text-sm font-medium transition-colors flex justify-between items-center ${activeYearId === year.id ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400' : 'hover:bg-zinc-50 dark:hover:bg-zinc-800/50 text-zinc-700 dark:text-zinc-300'}`}
                      >
                        {year.name}
              ...
              {/* Notifications */}
              <div className="relative group">
                <button className="p-2 relative text-zinc-500 hover:text-blue-600 dark:text-zinc-400 dark:hover:text-blue-400 transition-all duration-300 bg-white/50 hover:bg-white dark:bg-zinc-800/30 dark:hover:bg-zinc-800 backdrop-blur-md rounded-xl shadow-sm hover:shadow-md border border-white/40 dark:border-white/5">
                ...
                {/* Dropdown */}
                <div className="absolute right-0 mt-3 w-80 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-2xl rounded-2xl shadow-2xl border border-white/40 dark:border-zinc-700/50 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50 transform origin-top-right scale-95 group-hover:scale-100">
                  ...
                  <div className="max-h-80 overflow-y-auto">
                    <div className="p-4 border-b border-zinc-50 dark:border-zinc-800/50 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 cursor-pointer transition-colors">
                      <div className="flex items-start gap-3">
                        <div className="mt-1 shrink-0 h-2 w-2 rounded-full bg-blue-500"></div>
                        <div>
                          <p className="text-sm text-zinc-800 dark:text-zinc-200 font-medium">New Instagram DM</p>
                          <p className="text-xs text-zinc-500 mt-1">John asked about the upcoming event.</p>
                    ...
                    <div className="p-4 border-b border-zinc-50 dark:border-zinc-800/50 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 cursor-pointer transition-colors">
                      <div className="flex items-start gap-3">
                        <div className="mt-1 shrink-0 h-2 w-2 rounded-full bg-blue-500"></div>
                        <div>
                          <p className="text-sm text-zinc-800 dark:text-zinc-200 font-medium">Post Published Successfully</p>
                          <p className="text-xs text-zinc-500 mt-1">Your LinkedIn post is now live.</p>
                          <p className="text-[10px] text-zinc-400 mt-2 uppercase font-semibold">1 hour ago</p>
                        </div>
                      </div>
                    </div>
                    <div className="p-4 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 cursor-pointer transition-colors">
                      <div className="flex items-start gap-3">
                        <div className="mt-1 shrink-0 h-2 w-2 rounded-full bg-zinc-300"></div>
                        <div>
                          <p className="text-sm text-zinc-800 dark:text-zinc-200 font-medium">New Channel Connected</p>
                          <p className="text-xs text-zinc-500 mt-1">Facebook Page was added to the hub.</p>
                  ...
                  <div className="p-3 border-t border-zinc-100 dark:border-zinc-800 text-center">
                    <button className="text-sm text-zinc-500 hover:text-zinc-700 font-medium">View all notifications</button>
                  </div>
                </div>
              </div>

              {/* User Profile & Logout */}
              <div className="hidden sm:flex items-center gap-3 cursor-pointer group bg-white/50 hover:bg-white dark:bg-zinc-800/30 dark:hover:bg-zinc-800 backdrop-blur-md px-2 py-1.5 rounded-2xl transition-all duration-300 shadow-sm hover:shadow-md border border-white/40 dark:border-white/5">
