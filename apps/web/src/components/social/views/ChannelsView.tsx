...
export default function ChannelsView() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Social Channels</h1>
        <p className="text-slate-500 dark:text-slate-400">Connect and manage your brand&apos;s social media profiles.</p>
      </div>

      <div>
        <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-4">Connected Channels</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {connectedChannels.map(channel => (
            <div key={channel.id} className="bg-white dark:bg-zinc-900/50 dark:backdrop-blur-md p-5 rounded-xl border border-slate-200 dark:border-zinc-800 shadow-sm flex flex-col justify-between">
              ...
              <div>
                <h3 className="font-bold text-slate-900 dark:text-slate-100">{channel.name}</h3>
                ...
                <div className="mt-4 pt-4 border-t border-slate-100 dark:border-zinc-800 flex gap-2">
                  <button onClick={() => setEditingChannel(channel)} className="flex-1 px-3 py-1.5 bg-slate-50 dark:bg-zinc-800/50 hover:bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-slate-300 text-sm font-medium rounded-md transition-colors">
                    Edit
                  </button>
      ...
      <div>
        <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-4">Connect New Channel</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {availableChannels.map(channel => (
            <div 
              key={channel.platform} 
              onClick={() => handleConnectClick(channel)}
              className="bg-white dark:bg-zinc-900/50 dark:backdrop-blur-md p-5 rounded-xl border border-slate-200 dark:border-zinc-800 shadow-sm flex items-center gap-4 hover:border-indigo-300 transition-colors cursor-pointer group"
            >
              <div className="h-12 w-12 rounded-lg bg-slate-50 dark:bg-zinc-800/50 border border-slate-100 dark:border-zinc-800 flex items-center justify-center group-hover:scale-105 transition-transform shrink-0">
                <channel.icon className={`h-6 w-6 ${channel.color}`} />
              </div>
              <div className="flex-1">
                <h3 className="font-bold text-slate-900 dark:text-slate-100">{channel.platform}</h3>
                <p className="text-sm text-slate-500 dark:text-slate-400 leading-tight">{channel.description}</p>
              </div>
              <button className="h-8 w-8 rounded-full bg-slate-50 dark:bg-zinc-800/50 flex items-center justify-center text-slate-400 group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-colors shrink-0">
      ...
      {authenticatingChannel && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-zinc-900/50 dark:backdrop-blur-md rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 text-center relative border-b border-slate-100 dark:border-zinc-800">
              <button 
                ...
                className="absolute right-4 top-4 text-slate-400 hover:text-slate-600 dark:text-slate-300 transition-colors"
              ...
              {oauthStep === 'intro' && (
                <>
                  <div className="mx-auto h-16 w-16 bg-slate-50 dark:bg-zinc-800/50 border border-slate-100 dark:border-zinc-800 rounded-2xl flex items-center justify-center mb-4 shadow-sm">
                    <authenticatingChannel.icon className={`h-8 w-8 ${authenticatingChannel.color}`} />
                  </div>
                  <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">Connect to {authenticatingChannel.platform}</h2>
              ...
              {oauthStep === 'login' && (
                <>
                  ...
                  <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">Sign in to {authenticatingChannel.platform}</h2>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">
              ...
              {oauthStep === 'permissions' && (
                <>
                  ...
                  <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">Authorize Application</h2>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">
            ...
            <div className="p-6 bg-slate-50 dark:bg-zinc-800/50 space-y-4">
              {oauthStep === 'intro' && (
                <button 
              ...
              {oauthStep === 'login' && (
                <div className="space-y-4">
                  <input type="email" placeholder="Email or Username" className="w-full border border-slate-300 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100 focus:ring-indigo-500 focus:border-indigo-500" />
                  <input type="password" placeholder="Password" className="w-full border border-slate-300 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100 focus:ring-indigo-500 focus:border-indigo-500" />
                  <button 
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-zinc-900/50 dark:backdrop-blur-md rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 relative border-b border-slate-100 dark:border-zinc-800">
              ...
              <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100">Edit Channel</h2>
            </div>
            ...
            <div className="p-6 bg-slate-50 dark:bg-zinc-800/50 space-y-4">
              <div className="space-y-4">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full border border-slate-300 rounded-lg px-3 py-2 text-slate-900 dark:text-slate-100 focus:ring-indigo-500 focus:border-indigo-500"
                  />
                </div>
                <div>
              ...
              <div className="flex justify-end gap-3 pt-4">
                <button 
                  onClick={() => setEditingChannel(null)}
                  className="px-4 py-2 text-slate-700 dark:text-slate-300 bg-white dark:bg-zinc-900/50 dark:backdrop-blur-md border border-slate-300 hover:bg-slate-50 dark:bg-zinc-800/50 rounded-lg font-medium transition-colors"
                >
                  Cancel
                </button>
                <button 
