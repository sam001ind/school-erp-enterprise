...
export default function CalendarView() {
    ...
    return (
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center rounded-md border border-slate-200 dark:border-zinc-800 overflow-hidden bg-white dark:bg-zinc-900/50 dark:backdrop-blur-md">
            ...
            <button className="px-3 py-1 hover:bg-slate-50 dark:bg-zinc-800/50 text-sm font-medium text-slate-500 dark:text-slate-400">Week</button>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button 
            onClick={() => setCurrentDate(subMonths(currentDate, 1))}
            className="p-1.5 border border-slate-200 dark:border-zinc-800 rounded text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:bg-zinc-800/50"
          >
            <ChevronLeft className="h-5 w-5" />
          </button>
          <button 
            onClick={() => setCurrentDate(new Date())}
            className="px-3 py-1.5 border border-slate-200 dark:border-zinc-800 rounded text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:bg-zinc-800/50"
          >
            Today
          </button>
          <button 
  ...
  const renderCells = () => {
          ...
          <div
            ...
            className={`min-h-[120px] p-2 border-b border-r border-slate-100 dark:border-zinc-800 transition-colors ${
              !isSameMonth(day, monthStart)
                ? "bg-slate-50 dark:bg-zinc-800/50/50 text-slate-400"
                : isSameDay(day, new Date())
                ? "bg-indigo-50/30 text-indigo-600"
                : "bg-white dark:bg-zinc-900/50 dark:backdrop-blur-md text-slate-700 dark:text-slate-300"
            }`}
          >
    ...
    return <div className="border-l border-t border-slate-100 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 dark:backdrop-blur-md rounded-b-xl overflow-hidden">{rows}</div>;
  };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 h-full flex flex-col">
      ...
      <div className="bg-white dark:bg-zinc-900/50 dark:backdrop-blur-md rounded-xl border border-slate-200 dark:border-zinc-800 shadow-sm flex-1 flex flex-col p-6">
        {renderHeader()}
        <div className="flex-1 flex flex-col rounded-xl border border-slate-200 dark:border-zinc-800 overflow-hidden">
          <div className="bg-slate-50 dark:bg-zinc-800/50">
            {renderDays()}
          </div>
