...
import { Shield, Plus, ChevronDown, ChevronRight, CheckSquare, Square, Trash2, Edit2, ShieldAlert } from "lucide-react";

export default function RoleManagementView() {
  const { roles, addRole, deleteRole, updateRole } = useUserManagement();
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8 pb-12 items-start">
      ...
      {/* Sidebar: Role List */}
      <div className="w-full md:w-80 bg-white dark:bg-zinc-900/50 backdrop-blur-xl border border-slate-200 dark:border-zinc-800 rounded-3xl overflow-hidden shadow-sm shrink-0">
        <div className="p-6 border-b border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-800/30 flex justify-between items-center">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <Shield className="h-5 w-5 text-indigo-500" /> Roles
          </h2>
          <button onClick={handleCreateRole} className="p-1.5 bg-white dark:bg-zinc-900 text-indigo-600 dark:text-indigo-400 rounded-lg shadow-sm border border-slate-200 dark:border-zinc-700 hover:bg-slate-50 transition-colors">
        ...
        <div className="divide-y divide-slate-100 dark:divide-zinc-800 max-h-[600px] overflow-y-auto">
          {roles.map(role => (
            <button 
              ...
              className={`w-full text-left p-5 transition-colors ${activeRoleId === role.id ? 'bg-indigo-50/50 dark:bg-indigo-900/10' : 'hover:bg-slate-50 dark:hover:bg-zinc-800/50'}`}
            >
              <div className="flex justify-between items-start mb-1">
                ...
                {role.id === 'role_admin' && <ShieldAlert className="h-4 w-4 text-rose-500" title="Super Admin Role" />}
              </div>
      ...
      <div className="flex-1 w-full bg-white dark:bg-zinc-900/50 backdrop-blur-xl border border-slate-200 dark:border-zinc-800 rounded-3xl shadow-sm overflow-hidden">
        {activeRole ? (
          <>
            <div className="p-8 border-b border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-800/30">
               <div className="flex justify-between items-start">
                 ...
                 <div className="flex gap-2">
                   {activeRole.id !== 'role_admin' && (
                     <button onClick={() => deleteRole(activeRole.id)} className="p-2 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-xl transition-colors">
            ...
            <div className="p-8">
              ...
              <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                {Object.entries(moduleMenus).map(([modulePath, menus]) => {
                  ...
                  return (
                    <div key={modulePath} className="border border-slate-200 dark:border-zinc-800 rounded-2xl overflow-hidden bg-white dark:bg-zinc-900/50">
                      
                      {/* Module Header Row */}
                      <div className="flex items-center p-4 bg-slate-50 dark:bg-zinc-800/50 hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors">
                        <button onClick={() => toggleModuleExpand(modulePath)} className="p-1 mr-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
                        ...
                        >
                          {isAllAllowed ? <CheckSquare className="h-5 w-5" /> : isSomeAllowed ? <div className="h-5 w-5 border-2 border-indigo-500 rounded bg-indigo-500 flex items-center justify-center"><div className="w-2.5 h-0.5 bg-white rounded-full"></div></div> : <Square className="h-5 w-5 text-slate-300 dark:text-zinc-600" />}
                        </button>
                      ...
                      {/* Children Menu Rows */}
                      {isExpanded && (
                        <div className="bg-white dark:bg-zinc-900 border-t border-slate-100 dark:border-zinc-800/50">
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-1 p-4 pl-14">
                            {menus.map(menu => {
                              ...
                              return (
                                <button 
                                  key={menu.id}
                                  onClick={() => toggleMenuAccess(menuPath)}
                                  className="flex items-center gap-3 p-2 text-left hover:bg-slate-50 dark:hover:bg-zinc-800/50 rounded-lg transition-colors group"
                                >
                                  {isAllowed ? (
                                    <CheckSquare className="h-4 w-4 text-indigo-600 dark:text-indigo-400 shrink-0" />
                                  ) : (
                                    <Square className="h-4 w-4 text-slate-300 dark:text-zinc-600 shrink-0 group-hover:text-slate-400" />
                                  )}
