  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-white dark:bg-zinc-900/50 backdrop-blur-xl p-6 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex gap-4 w-full md:w-auto">
          <select 
            ...
            className="bg-slate-50 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-slate-900 dark:text-white rounded-xl px-4 py-2.5 outline-none focus:border-indigo-500"
          >
        ...
        <div className="flex gap-3">
           <div className="flex bg-slate-100 dark:bg-zinc-800 p-1 rounded-xl">
              <button onClick={() => setViewMode("monthly")} className={`px-4 py-1.5 text-sm font-semibold rounded-lg transition-colors ${viewMode === 'monthly' ? 'bg-white dark:bg-zinc-700 shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-slate-500'}`}>Monthly</button>
              <button onClick={() => setViewMode("weekly")} className={`px-4 py-1.5 text-sm font-semibold rounded-lg transition-colors ${viewMode === 'weekly' ? 'bg-white dark:bg-zinc-700 shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-slate-500'}`}>Weekly</button>
           </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-zinc-900/50 backdrop-blur-xl p-6 rounded-3xl border border-slate-200 dark:border-zinc-800 shadow-sm flex flex-col justify-between">
      ...
      <div className="bg-white dark:bg-zinc-900/50 backdrop-blur-xl border border-slate-200 dark:border-zinc-800 shadow-sm rounded-3xl overflow-hidden">
        <div className="p-6 border-b border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-800/30">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">Detailed Lesson Plans</h2>
        </div>
        <div className="divide-y divide-slate-100 dark:divide-zinc-800">
          {lessonPlans.map(plan => (
            <div key={plan.id} className="p-6 flex flex-col md:flex-row md:items-center gap-6 hover:bg-slate-50/50 dark:hover:bg-zinc-800/20 transition-colors">
               <div className="flex-1">
                 <div className="flex items-center gap-3 mb-2">
                   <h3 className={`text-lg font-bold ${plan.completed ? 'text-slate-500 line-through dark:text-slate-400' : 'text-slate-900 dark:text-white'}`}>{plan.topic}</h3>
                 ...
                 <p className="text-sm text-slate-600 dark:text-slate-400 max-w-3xl">{plan.description}</p>
               ...
               <div className="flex items-center gap-4">
                  <button onClick={() => toggleLessonPlan(activeSubjectId, plan.id)} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-colors ${plan.completed ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-slate-100 text-slate-600 hover:bg-slate-200 dark:bg-zinc-800 dark:text-slate-400 dark:hover:bg-zinc-700'}`}>
