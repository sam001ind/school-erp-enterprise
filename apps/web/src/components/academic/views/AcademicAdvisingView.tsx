  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-zinc-900/50 backdrop-blur-xl p-6 rounded-3xl border border-slate-200 dark:border-zinc-800 shadow-sm flex flex-col justify-between">
           ...
           <div className="space-y-4 flex-1">
             <div className="flex items-center gap-4 p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-zinc-800 transition-colors border border-transparent hover:border-slate-200 dark:hover:border-zinc-700">
                <div className="w-12 h-12 rounded-full bg-slate-100 dark:bg-zinc-800 flex items-center justify-center text-slate-500 font-bold shrink-0">JD</div>
                <div className="flex-1">
                  <h4 className="text-sm font-bold text-slate-900 dark:text-white">John Doe (10-A)</h4>
                  <p className="text-xs text-slate-500 flex items-center gap-1 mt-1"><CalendarIcon className="h-3 w-3" /> Today, 2:00 PM</p>
                </div>
             ...
             <div className="flex items-center gap-4 p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-zinc-800 transition-colors border border-transparent hover:border-slate-200 dark:hover:border-zinc-700">
                <div className="w-12 h-12 rounded-full bg-slate-100 dark:bg-zinc-800 flex items-center justify-center text-slate-500 font-bold shrink-0">AS</div>
                <div className="flex-1">
                  <h4 className="text-sm font-bold text-slate-900 dark:text-white">Alice Smith (11-B)</h4>
                  <p className="text-xs text-slate-500 flex items-center gap-1 mt-1"><CalendarIcon className="h-3 w-3" /> Tomorrow, 10:30 AM</p>
                </div>
                <button className="p-2 bg-indigo-50 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400 rounded-lg"><MessageSquare className="h-4 w-4" /></button>
             </div>
           </div>
      ...
      <div className="bg-white dark:bg-zinc-900/50 backdrop-blur-xl border border-slate-200 dark:border-zinc-800 shadow-sm rounded-3xl overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0 divide-y md:divide-y-0 md:divide-x divide-slate-100 dark:divide-zinc-800">
          {faculty.slice(0,3).map(f => (
            <div key={f.id} className="p-6">
              ...
              <div className="flex justify-between items-center bg-slate-50 dark:bg-zinc-800 rounded-xl p-3 mb-4 border border-slate-100 dark:border-zinc-700">
                <span className="text-xs font-bold text-slate-500 uppercase">Assigned Mentees</span>
                <span className="font-bold text-slate-900 dark:text-white">12</span>
              </div>
              <button className="w-full py-2 border border-slate-200 dark:border-zinc-700 rounded-xl text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-zinc-800 transition-colors">
                View Profile
              </button>
            </div>
          ))}
