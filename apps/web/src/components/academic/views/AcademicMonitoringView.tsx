    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-white dark:bg-zinc-900/50 backdrop-blur-xl p-6 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex gap-2">
          <button onClick={() => setFilter("all")} className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${filter === 'all' ? 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-zinc-800'}`}>All Classes</button>
          <button onClick={() => setFilter("critical")} className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${filter === 'critical' ? 'bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-zinc-800'}`}>Needs Attention</button>
        </div>
        
        <button className="w-full md:w-auto bg-slate-100 hover:bg-slate-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-slate-700 dark:text-slate-300 px-5 py-2.5 rounded-xl font-semibold shadow-sm transition-colors flex items-center justify-center gap-2 border border-slate-200 dark:border-zinc-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {classes.map(c => {
          ...
          return (
            <div key={c.id} className="bg-white dark:bg-zinc-900/50 backdrop-blur-xl rounded-3xl border border-slate-200 dark:border-zinc-800 shadow-sm p-6 hover:shadow-md transition-shadow">
              ...
              <div className="space-y-4">
                <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">Subject Breakdown</p>
                {c.subjects.map(sub => {
                  ...
                  return (
                    <div key={sub.id} className="bg-slate-50 dark:bg-zinc-800/50 p-4 rounded-xl border border-slate-100 dark:border-zinc-700">
                      <div className="flex justify-between text-sm mb-2">
                        <span className="font-bold text-slate-900 dark:text-white">{sub.name}</span>
                        <span className="text-slate-500 font-semibold">{subCompleted}/{subTotal} Plans</span>
                      </div>
