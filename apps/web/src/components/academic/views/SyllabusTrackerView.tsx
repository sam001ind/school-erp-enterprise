  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 bg-white dark:bg-zinc-900/50 backdrop-blur-xl p-6 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-sm flex flex-col justify-between">
          ...
          <div className="flex gap-4">
            <select 
              ...
              className="bg-slate-50 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-slate-900 dark:text-white rounded-xl px-4 py-3 outline-none focus:border-indigo-500 flex-1"
            >
              {classes.map(c => (
        ...
        {/* Syllabus Progress */}
        <div className="bg-white dark:bg-zinc-900/50 backdrop-blur-xl p-6 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-sm flex flex-col items-center justify-center">
          ...
          <div className="space-y-4">
            {lessonPlans.map((plan, index) => (
              <div key={plan.id} className="relative flex gap-4">
                ...
                {/* Content */}
                <div className={`flex-1 p-4 rounded-xl border ${plan.completed ? 'bg-slate-50 border-slate-200 dark:bg-zinc-800/30 dark:border-zinc-700/50 opacity-75' : 'bg-white border-slate-200 dark:bg-zinc-900 dark:border-zinc-700 shadow-sm'} transition-all`}>
                  <div className="flex justify-between items-start mb-2">
                    <h4 className={`text-base font-bold ${plan.completed ? 'text-slate-600 dark:text-slate-400 line-through' : 'text-slate-900 dark:text-white'}`}>
                      {plan.topic}
                    </h4>
