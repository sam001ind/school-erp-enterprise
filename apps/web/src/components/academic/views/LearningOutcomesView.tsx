    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredOutcomes.map(outcome => (
          <div key={outcome.id} className="bg-white dark:bg-zinc-900/50 backdrop-blur-xl rounded-3xl border border-slate-200 dark:border-zinc-800 shadow-sm p-6 hover:shadow-md transition-shadow">
            <div className="flex justify-between items-start mb-4">
              ...
              <button className="text-slate-400 hover:text-indigo-600 transition-colors"><Target className="h-5 w-5" /></button>
            </div>
            
            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-6 line-clamp-2 min-h-[56px]">{outcome.description}</h3>
            
            <div className="bg-slate-50 dark:bg-zinc-800/50 rounded-2xl p-4 border border-slate-100 dark:border-zinc-700/50">
              ...
              <div className="flex flex-wrap gap-2">
                {outcome.mappedSubjects.map(subCode => {
                  return (
                    <span key={subCode} className="px-2.5 py-1 text-xs font-bold rounded-md bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 text-slate-700 dark:text-slate-300 shadow-sm">
                      {subCode}
                    </span>
                  );
                })}
                {outcome.mappedSubjects.length === 0 && (
                  <span className="text-sm text-slate-400 italic">No subjects mapped yet.</span>
                )}
              </div>
            </div>
            ...
            <div className="mt-6 flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-slate-500">75% Attained</span>
              </div>
