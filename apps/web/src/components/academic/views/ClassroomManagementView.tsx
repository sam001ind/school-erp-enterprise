    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredRooms.map(room => (
          <div key={room.id} className="bg-white dark:bg-zinc-900/50 backdrop-blur-xl rounded-3xl border border-slate-200 dark:border-zinc-800 shadow-sm p-6 hover:shadow-md transition-shadow group">
            <div className="flex justify-between items-start mb-6">
              ...
              <div className="flex flex-col items-end gap-2">
                <span className={`px-2.5 py-1 text-xs font-bold rounded-lg border ${room.type === 'Laboratory' ? 'bg-purple-50 text-purple-700 border-purple-200 dark:bg-purple-900/30 dark:text-purple-400 dark:border-purple-800' : 'bg-slate-100 text-slate-700 border-slate-200 dark:bg-zinc-800 dark:text-slate-300 dark:border-zinc-700'}`}>
                  {room.type}
                </span>
                {room.isSmart && (
            ...
            <div className="bg-slate-50 dark:bg-zinc-800 p-4 rounded-xl flex justify-between items-center border border-slate-100 dark:border-zinc-700">
