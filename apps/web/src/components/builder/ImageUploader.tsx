        ...
        <div className="relative group">
          ...
          <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity rounded-md flex items-center justify-center gap-2 backdrop-blur-sm">
            <button 
              ...
              className="bg-white text-zinc-900 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-zinc-200"
            >
              Change
            </button>
            <button 
          ...
          {fileSize && <p className="text-xs text-zinc-500 mt-2 text-center">File Size: {fileSize}</p>}
        </div>
      ) : (
        <div 
          onClick={() => fileInputRef.current?.click()}
          className="border-2 border-dashed border-zinc-300 dark:border-zinc-700 rounded-lg p-6 flex flex-col items-center justify-center cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors"
        >
          <span className="text-2xl mb-2">📸</span>
          <p className="text-sm font-medium text-zinc-600 dark:text-zinc-400">Click to upload image</p>
