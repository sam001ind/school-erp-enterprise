...
const modulesToUpgrade = [
  { name: 'academic', title: 'Academic Management', desc: 'Manage classes, subjects, and curriculum.', icon: 'BookOpen' },
  { name: 'examinations', title: 'Examinations', desc: 'Schedule exams, grade students, and publish results.', icon: 'Award' },
  { name: 'fees', title: 'Fee Management', desc: 'Track invoices, payments, and financial reports.', icon: 'CreditCard' },
  { name: 'hostel', title: 'Hostel & Dorms', desc: 'Manage rooms, allocations, and warden tasks.', icon: 'Home' },
  { name: 'leave', title: 'Leave Management', desc: 'Approve and track employee and student leaves.', icon: 'CalendarDays' },
  { name: 'library', title: 'Library System', desc: 'Manage book inventory, issues, and returns.', icon: 'Library' },
  { name: 'payroll', title: 'Payroll', desc: 'Manage employee salaries, taxes, and payslips.', icon: 'Banknote' },
  { name: 'transport', title: 'Transport', desc: 'Manage vehicles, routes, and tracking.', icon: 'Bus' }
];

const template = (mod) => `"use client";
import React, { Suspense } from "react";
import { useSearchParams } from "next/navigation";
import { ${mod.icon}, Search, Filter, Plus, ArrowUpRight, ArrowDownRight, MoreVertical } from "lucide-react";

function ${mod.title.replace(/\s+/g, '').replace(/&/g, '')}Content() {
    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-2">${mod.desc}</p>
        </div>
...
export default function ${mod.title.replace(/\s+/g, '').replace(/&/g, '')}Dashboard() {
  return (
    <Suspense fallback={<div className="p-8 text-center text-slate-500">Loading module...</div>}>
      <${mod.title.replace(/\s+/g, '').replace(/&/g, '')}Content />
    </Suspense>
  )
}
`;

modulesToUpgrade.forEach(mod => {
  const filePath = path.join(__dirname, 'apps/web/src/app/(dashboard)/' + mod.name + '/page.tsx');
  if (fs.existsSync(filePath)) {
    fs.writeFileSync(filePath, template(mod));
    console.log('Upgraded ' + mod.name + ' with Luminous Dashboard template!');
  }
});
